...
__global__ void k_InitDescending(unsigned int* sort, const int size) {
  for (auto i = threadIdx.x + blockDim.x * blockIdx.x; i < size;
...
__global__ void k_InitRandom(unsigned int* sort,
                             const int size,
                             const int seed) {
